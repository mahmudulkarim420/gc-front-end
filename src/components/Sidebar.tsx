...
export function Sidebar({
  ...
  return (
    <>
      <nav
        className={`fixed left-0 top-0 h-full flex flex-col z-50 bg-[#0F172A] font-manrope antialiased text-sm tracking-tight w-64 border-r border-slate-800/50 transition-all duration-300 ease-in-out ${
          isOpen
      ...
      >
        <div className="px-6 py-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 uppercase tracking-widest font-black text-[9px]">
              CollabChat Pro
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 space-y-8 custom-scrollbar">
          ...
          <div className="space-y-1">
            <Link
              onClick={onClose}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:text-emerald-400 hover:bg-slate-800/50 transition-all text-slate-400"
              href="/chat"
            >
              ...
              {groups.map((group) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-lg transition-all group text-left ${
                      isActive
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-6 h-6 rounded bg-slate-800 flex items-center justify-center overflow-hidden border ${
                      ...
                      >
                        {group.imageUrl ? (
                          ...
                          <span
                            className={`text-[10px] font-bold ${isActive ? "text-emerald-400" : "text-emerald-500/50"}`}
                          >
                            #
                          </span>
                        )}
                ...
                return (
                  ...
                  >
                    <div className="relative w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-emerald-500 border border-white/5 overflow-hidden">
                      ...
                      <div
                        className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-[#0F172A] ${
                          isOnline
                    ...
                    <span className={`text-sm ${isOnline ? "text-slate-200" : "text-slate-500"}`}>
                      {member.username}
                    </span>
                  </div>
                );
